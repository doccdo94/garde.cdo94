...
function afficherDocumentsEtTemplates(cont) {
  ...
  // Section RGPD conservation
  html += '<div class="doc-section" style="border-color:#f59e0b;margin-top:20px"><h3>🔒 RGPD — Conservation des données</h3><p class="doc-section-desc">Visualisez et purgez les données de plus de 2 ans (conformément à la politique de confidentialité).</p><div id="rgpd-data-container"><div class="loading"><div class="spinner"></div></div></div></div>';
...
async function chargerRGPD() {
  const cont = document.getElementById('rgpd-data-container');
  if (!cont) return;
  try {
    const r = await fetch('/api/rgpd/donnees-anciennes');
    const d = await r.json();
    const total = d.inscriptions.nb + d.campagnes.nb + d.email_events.nb;
    if (total === 0) {
      cont.innerHTML = '<p style="color:#10b981;font-weight:600">✅ Aucune donnée de plus de 2 ans. Rien à purger.</p>';
      return;
    }
    cont.innerHTML = `
      <p style="margin-bottom:12px;font-size:14px;color:#374151">Données antérieures au <strong>${new Date(d.limite).toLocaleDateString('fr-FR')}</strong> :</p>
      <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:12px;margin-bottom:16px">
        <div style="background:#fef2f2;border:2px solid #fecaca;border-radius:10px;padding:16px;text-align:center">
          <div style="font-size:28px;font-weight:700;color:#dc2626">${d.inscriptions.nb}</div>
          <div style="font-size:12px;color:#991b1b;font-weight:600">INSCRIPTIONS</div>
          ${d.inscriptions.nb>0?`<button class="btn btn-danger" style="margin-top:10px;font-size:11px;padding:6px 12px" onclick="purgerRGPD('inscriptions')">🗑️ Purger</button>`:''}
        </div>
        <div style="background:#fef2f2;border:2px solid #fecaca;border-radius:10px;padding:16px;text-align:center">
          <div style="font-size:28px;font-weight:700;color:#dc2626">${d.campagnes.nb}</div>
          <div style="font-size:12px;color:#991b1b;font-weight:600">CAMPAGNES</div>
          ${d.campagnes.nb>0?`<button class="btn btn-danger" style="margin-top:10px;font-size:11px;padding:6px 12px" onclick="purgerRGPD('campagnes')">🗑️ Purger</button>`:''}
        </div>
        <div style="background:#fef2f2;border:2px solid #fecaca;border-radius:10px;padding:16px;text-align:center">
          <div style="font-size:28px;font-weight:700;color:#dc2626">${d.email_events.nb}</div>
          <div style="font-size:12px;color:#991b1b;font-weight:600">ÉVÉNEMENTS EMAIL</div>
          ${d.email_events.nb>0?`<button class="btn btn-danger" style="margin-top:10px;font-size:11px;padding:6px 12px" onclick="purgerRGPD('events')">🗑️ Purger</button>`:''}
        </div>
      </div>
      ${total>0?`<button class="btn btn-danger" onclick="purgerRGPD('tout')" style="width:100%">🗑️ Tout purger (${total} enregistrements de plus de 2 ans)</button>`:''}
    `;
  } catch(e) { cont.innerHTML = '<p style="color:#ef4444">Erreur de chargement</p>'; }
}

async function purgerRGPD(type) {
  const label = type==='tout'?'TOUTES les données':'les '+type;
  if (!confirm(`⚠️ Supprimer ${label} de plus de 2 ans ?\\n\\nCette action est irréversible.`)) return;
  try {
    if (type === 'tout' || type === 'inscriptions') {
      const r = await fetch('/api/rgpd/purger-inscriptions',{method:'DELETE'});
      const d = await r.json(); if (d.nb_supprimees>0) afficherMessage(`${d.nb_supprimees} inscriptions purgées`);
    }
    if (type === 'tout' || type === 'campagnes') {
      const r = await fetch('/api/rgpd/purger-campagnes',{method:'DELETE'});
      const d = await r.json(); if (d.nb_campagnes>0) afficherMessage(`${d.nb_campagnes} campagnes purgées`);
    }
    if (type === 'tout' || type === 'events') {
      const r = await fetch('/api/rgpd/purger-events',{method:'DELETE'});
      const d = await r.json(); if (d.nb_supprimees>0) afficherMessage(`${d.nb_supprimees} événements purgés`);
    }
    chargerRGPD();
