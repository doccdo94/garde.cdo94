...
app.get('/api/diagnostic', async (req, res) => {
  ...
  if (rapport.base_de_donnees.connexion === 'OK') {
    for (const type of ['confirmation', 'rappel_j7', 'rappel_j1', 'annulation']) {
      try {
        ...
        SELECT
          ...
          (SELECT COUNT(*) FROM inscriptions
             WHERE date_garde = CURRENT_DATE + INTERVAL '7 days'
               AND (email_rappel_j7_statut IS NULL OR email_rappel_j7_statut <> 'envoye')) AS rappels_j7_en_attente,
          (SELECT COUNT(*) FROM inscriptions
             WHERE date_garde = CURRENT_DATE + INTERVAL '1 day'
               AND (email_rappel_j1_statut IS NULL OR email_rappel_j1_statut <> 'envoye')) AS rappels_j1_en_attente,
          (SELECT COUNT(*) FROM inscriptions
