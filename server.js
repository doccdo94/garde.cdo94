...
function assemblerEmailHTML(template, variables) {
  ...
  const baseUrl = process.env.RENDER_EXTERNAL_URL || process.env.BASE_URL || 'https://garde-cdo94.onrender.com';
  const html = `<!DOCTYPE html><html><head><meta charset="UTF-8"></head><body style="font-family:Arial,sans-serif;line-height:1.6;color:#333;margin:0;padding:0"><div style="max-width:600px;margin:0 auto;padding:20px"><div style="background:linear-gradient(135deg,${couleur1} 0%,${couleur2} 100%);color:white;padding:30px;text-align:center;border-radius:10px 10px 0 0"><h1 style="margin:0;font-size:24px">${titre_header}</h1>${sous_titre_header?`<p style="margin:10px 0 0 0;font-size:18px">${sous_titre_header}</p>`:''}</div><div style="background:#f9f9f9;padding:30px;border-radius:0 0 10px 10px">${contenu_html}</div><div style="text-align:center;margin-top:30px;color:#999;font-size:11px;line-height:1.5"><p style="margin:0 0 6px 0;color:#666;font-size:12px">CDO 94 — Conseil Départemental de l'Ordre des Chirurgiens-Dentistes du Val-de-Marne</p><p style="margin:0"><a href="${baseUrl}/confidentialite.html" style="color:#667eea;text-decoration:none">Politique de confidentialité</a> · Données protégées conformément au RGPD</p></div></div></body></html>`;
  return { sujet, html };
}
...
// Vue des données anciennes (> 2 ans)
app.get('/api/rgpd/donnees-anciennes', requireAuth, async (req, res) => {
  try {
    const limiteDate = new Date();
    limiteDate.setFullYear(limiteDate.getFullYear() - 2);
    const limite = limiteDate.toISOString().split('T')[0];

    const inscR = await pool.query(
      `SELECT COUNT(*) as nb, MIN(date_garde)::text as plus_ancienne, MAX(date_garde)::text as plus_recente
       FROM inscriptions WHERE date_garde < $1`, [limite]);

    const campR = await pool.query(
      `SELECT COUNT(*) as nb, MIN(lancee_at)::text as plus_ancienne
       FROM campagnes WHERE lancee_at < $1`, [limite]);

    const eventsR = await pool.query(
      `SELECT COUNT(*) as nb FROM email_events WHERE date_event < $1`, [limite]);

    res.json({
      limite,
      inscriptions: { nb: parseInt(inscR.rows[0].nb), plus_ancienne: inscR.rows[0].plus_ancienne, plus_recente: inscR.rows[0].plus_recente },
      campagnes: { nb: parseInt(campR.rows[0].nb), plus_ancienne: campR.rows[0].plus_ancienne },
      email_events: { nb: parseInt(eventsR.rows[0].nb) }
    });
  } catch(e) { res.status(500).json({error:'Erreur'}); }
});

// Purge manuelle des inscriptions > 2 ans
app.delete('/api/rgpd/purger-inscriptions', requireAuth, async (req, res) => {
  try {
    const limiteDate = new Date();
    limiteDate.setFullYear(limiteDate.getFullYear() - 2);
    const limite = limiteDate.toISOString().split('T')[0];
...
// Purge manuelle des campagnes + destinataires > 2 ans
app.delete('/api/rgpd/purger-campagnes', requireAuth, async (req, res) => {
  try {
    const limiteDate = new Date();
    limiteDate.setFullYear(limiteDate.getFullYear() - 2);
    const limite = limiteDate.toISOString().split('T')[0];
    const campR = await pool.query('SELECT id FROM campagnes WHERE lancee_at < $1', [limite]);
    const ids = campR.rows.map(r => r.id);
    let nbDest = 0;
    if (ids.length > 0) {
      const destR = await pool.query('DELETE FROM campagne_destinataires WHERE campagne_id = ANY($1)', [ids]);
      nbDest = destR.rowCount;
      await pool.query('DELETE FROM campagnes WHERE id = ANY($1)', [ids]);
    }
    console.log(`🧹 RGPD: ${ids.length} campagnes et ${nbDest} destinataires de plus de 2 ans supprimés`);
    res.json({success:true, nb_campagnes: ids.length, nb_destinataires: nbDest});
  } catch(e) { res.status(500).json({error:'Erreur'}); }
});

// Purge manuelle des email_events > 2 ans
app.delete('/api/rgpd/purger-events', requireAuth, async (req, res) => {
  try {
    const limiteDate = new Date();
    limiteDate.setFullYear(limiteDate.getFullYear() - 2);
    const limite = limiteDate.toISOString().split('T')[0];
    const r = await pool.query('DELETE FROM email_events WHERE date_event < $1', [limite]);
    console.log(`🧹 RGPD: ${r.rowCount} email_events de plus de 2 ans supprimés`);
    res.json({success:true, nb_supprimees: r.rowCount});
  } catch(e) { res.status(500).json({error:'Erreur'}); }
});
...
// ============================================================
// ==========  BLOC DIAGNOSTIC — à coller AVANT app.listen  ===
// ============================================================
// Aucune dépendance nouvelle. N'écrit rien en base.
// 3 routes :
//   GET /health                        -> public, ultra-léger (monitoring externe)
//   GET /api/diagnostic?token=XXX      -> rapport complet, aucun email envoyé
//   GET /api/diagnostic?token=XXX&email=moi@x.fr  -> + 1 email de test réel
// ============================================================

// --- 1. Sonde publique pour UptimeRobot / anti spin-down ---
app.get('/health', (req, res) => {
  res.json({ ok: true, ts: new Date().toISOString() });
});

// --- 2. Diagnostic complet (protégé par ACCESS_TOKEN) ---
app.get('/api/diagnostic', async (req, res) => {
  ...
  const ko = (msg) => { rapport.alertes.push(msg); rapport.verdict = 'PROBLEME'; };
  const warn = (msg) => { rapport.alertes.push('⚠️ ' + msg); if (rapport.verdict === 'OK') rapport.verdict = 'ATTENTION'; };

  // ---------- A. Variables d'environnement ----------
  // On ne renvoie JAMAIS les valeurs, uniquement leur présence.
  const requises = ['DATABASE_URL', 'SUPABASE_URL', 'SUPABASE_SERVICE_KEY', 'BREVO_API_KEY'];
  requises.forEach(v => {
    const ok = !!process.env[v];
    rapport.environnement[v] = ok ? 'definie' : 'MANQUANTE';
    if (!ok) ko(`Variable ${v} absente`);
  });

  const defauts = {
    ACCESS_TOKEN: 'garde2027cdo94',
    ADMIN_PASSWORD: 'cdo94admin2025',
    SESSION_SECRET: 'cdo94-secret-session-key-change-me'
  };
  for (const [nom, valeurDefaut] of Object.entries(defauts)) {
    const utiliseDefaut = !process.env[nom] || process.env[nom] === valeurDefaut;
    rapport.environnement[nom] = utiliseDefaut ? 'VALEUR PAR DEFAUT DU CODE' : 'personnalisee';
    if (utiliseDefaut) warn(`${nom} utilise la valeur par défaut visible dans le dépôt GitHub`);
  }

  rapport.environnement.NODE_ENV = process.env.NODE_ENV || 'non definie';
  if (process.env.NODE_ENV !== 'production') {
    warn('NODE_ENV != production : cookies non sécurisés et SSL Postgres désactivé');
  }
  rapport.environnement.EMAIL_FROM = EMAIL_FROM;
  rapport.environnement.ADMIN_EMAIL = ADMIN_EMAIL;

  // ---------- B. Base de données ----------
  try {
    const tPing = Date.now();
    await pool.query('SELECT 1');
    rapport.base_de_donnees.connexion = 'OK';
    rapport.base_de_donnees.latence_ms = Date.now() - tPing;
    ...
    const manquantes = attendues.filter(t => !presentes.includes(t));
    rapport.base_de_donnees.tables_presentes = presentes.length + '/' + attendues.length;
    if (manquantes.length) {
      rapport.base_de_donnees.tables_manquantes = manquantes;
      ko('Tables absentes : ' + manquantes.join(', '));
    }
  } catch (e) {
    rapport.base_de_donnees.connexion = 'ECHEC';
    rapport.base_de_donnees.erreur = e.code === 'ENOTFOUND'
      ? 'ENOTFOUND — le projet Supabase est probablement en pause'
      : e.message;
    ko('Base de données injoignable');
  }
  ...
  if (rapport.base_de_donnees.connexion === 'OK') {
    for (const type of ['confirmation', 'rappel_j7', 'rappel_j1', 'annulation', 'invitation']) {
      try {
        ...
        if (r.rows.length) {
          rapport.templates[type] = 'en base';
        } else {
          rapport.templates[type] = 'fallback code';
          warn(`Template "${type}" absent en base, repli sur TEMPLATES_DEFAUT`);
        }
      } catch (e) {
        rapport.templates[type] = 'erreur lecture';
        ko(`Lecture du template "${type}" impossible`);
      }
    }
  }
  ...
  // ---------- D. Brevo ----------
  if (!BREVO_API_KEY) {
    rapport.brevo.statut = 'CLE ABSENTE';
  } else {
    try {
      const rc = await fetch('https://api.brevo.com/v3/account', {
        headers: { 'api-key': BREVO_API_KEY }
      });
      rapport.brevo.http = rc.status;
      if (rc.ok) {
        const acc = await rc.json();
        rapport.brevo.statut = 'CLE VALIDE';
        rapport.brevo.compte = acc.email;
        rapport.brevo.plan = (acc.plan || []).map(p => p.type).join(', ') || 'inconnu';
        const credits = (acc.plan || []).reduce((s, p) => s + (p.credits || 0), 0);
        rapport.brevo.credits_restants = credits;
        if (credits > 0 && credits < 50) warn(`Seulement ${credits} crédits Brevo restants`);
      } else {
        const txt = await rc.text();
        rapport.brevo.statut = 'CLE REFUSEE';
        rapport.brevo.reponse = txt.slice(0, 300);
        if (/unrecognised IP|unauthorized IP/i.test(txt)) ko('Brevo : IP Render non autorisée');
        else if (rc.status === 401) ko('Brevo : clé API invalide, révoquée ou désactivée');
        else if (rc.status === 402) ko('Brevo : crédits épuisés');
        else ko('Brevo : erreur HTTP ' + rc.status);
      }
    } catch (e) {
      rapport.brevo.statut = 'INJOIGNABLE';
      rapport.brevo.erreur = e.message;
      ko('Appel à api.brevo.com impossible');
    }
  }
    ...
    try {
      const q = await pool.query(`
        SELECT
          ...
          (SELECT COUNT(*) FROM dates_garde)  AS dates_garde,
          (SELECT COUNT(*) FROM campagnes)    AS campagnes,
          (SELECT COUNT(*) FROM email_events) AS email_events,
          -- Filtre identique à envoyerRappels() : NULL ou 'non_envoye' uniquement
          (SELECT COUNT(*) FROM inscriptions
             WHERE date_garde = CURRENT_DATE + INTERVAL '7 days'
               AND (email_rappel_j7_statut IS NULL OR email_rappel_j7_statut = 'non_envoye')) AS rappels_j7_en_attente,
          (SELECT COUNT(*) FROM inscriptions
             WHERE date_garde = CURRENT_DATE + INTERVAL '1 day'
               AND (email_rappel_j1_statut IS NULL OR email_rappel_j1_statut = 'non_envoye')) AS rappels_j1_en_attente,
          -- Statuts 'erreur' : jamais repris automatiquement, renvoi manuel obligatoire
          (SELECT COUNT(*) FROM inscriptions
             WHERE email_confirmation_statut = 'erreur') AS confirmations_en_erreur,
          (SELECT COUNT(*) FROM inscriptions
             WHERE date_garde >= CURRENT_DATE
               AND (email_rappel_j7_statut = 'erreur' OR email_rappel_j1_statut = 'erreur')) AS rappels_bloques_en_erreur
      `);
      rapport.donnees = q.rows[0];
      const enAttente = Number(q.rows[0].rappels_j7_en_attente) + Number(q.rows[0].rappels_j1_en_attente);
      if (enAttente > 0) {
        warn(`${enAttente} rappel(s) partiront automatiquement 10s après le prochain redémarrage`);
      }
      const bloques = Number(q.rows[0].rappels_bloques_en_erreur);
      if (bloques > 0) {
        warn(`${bloques} rappel(s) en statut "erreur" : jamais renvoyés automatiquement, renvoi manuel requis depuis l'admin`);
      }
    } catch (e) {
      rapport.donnees.erreur = e.message;
    }
  }
  ...
  rapport.process = {
    ...
    memoire_mb: Math.round(process.memoryUsage().rss / 1048576),
    demarrage_recent: process.uptime() < 120 ? 'oui (cold start probable)' : 'non'
  };

  // ---------- G. Email de test réel (optionnel) ----------
  if (req.query.email) {
    if (!validerEmail(req.query.email)) {
      rapport.email_test = { statut: 'adresse invalide' };
    } else {
      try {
        const tpl = await getTemplate('confirmation');
        const faux = {
          praticien_nom: 'TEST', praticien_prenom: 'Diagnostic',
          praticien_email: req.query.email, praticien_telephone: '0100000000',
          praticien_numero: '1', praticien_voie: 'rue de Test',
          praticien_code_postal: '94120', praticien_ville: 'Fontenay-sous-Bois'
        };
        const dateF = formatDateFr(new Date());
        const { sujet, html } = assemblerEmailHTML(tpl, buildVars(faux, dateF));
        const r = await envoyerEmailAvecPJ(
          req.query.email,
          '[TEST DIAGNOSTIC] ' + sujet,
          html, tpl,
          { nom: 'TEST', prenom: 'Diagnostic', dateGarde: dateF }
        );
        rapport.email_test = {
          destinataire: req.query.email,
          statut: r.success ? 'ENVOYE' : 'ECHEC',
          messageId: r.messageId,
          note: 'Copie envoyée en CC à ' + ADMIN_EMAIL + ' — 1 crédit Brevo consommé'
        };
        if (!r.success) ko('Envoi email de test échoué (voir les logs pour la réponse Brevo)');
      } catch (e) {
        rapport.email_test = { statut: 'EXCEPTION', erreur: e.message };
        ko('Exception pendant l\'email de test');
      }
    }
  }

  rapport.duree_totale_ms = Date.now() - t0;
  console.log(`🩺 Diagnostic: ${rapport.verdict} (${rapport.alertes.length} alerte(s), ${rapport.duree_totale_ms}ms)`);
  res.status(rapport.verdict === 'PROBLEME' ? 503 : 200).json(rapport);
});
